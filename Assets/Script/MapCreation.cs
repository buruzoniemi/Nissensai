    ...
    bool MapGenerate(int randomPattern)
    {
        if (randomPattern != 4)
        {
            for (int i = 0; i < map.GetLength(0); i++)
            {
                for (int j = 0; j < map.GetLength(1); j++)
                {
                    map[i, j] = Random.Range(0, 5); // マスに0, 1, 2を代入
                }
            }
        }
        else
        {
            ...
            switch (randomTerms)
            {
              case 1:
                 map = new int[,] {
                 {1,2,1,4,2,4}, {2,1,2,2,4,2}, {2,1,4,3,1,2},
                 {2,4,3,1,4,2}, {1,2,1,4,2,3}, {4,3,2,2,1,1}
             };
                 break;
              case 2:
                  map = new int[,] {
                 {2,4,2,3,2,4}, {4,1,4,2,1,3}, {1,3,1,2,4,1},
                 {4,1,1,2,1,4}, {1,2,3,2,1,4}, {3,1,2,4,3,1}
             };
                 break;
              case 3:
                  map = new int[,] {
                 {2,4,2,2,4,1}, {1,2,1,4,1,1}, {1,2,2,2,4,3},
                 {3,2,1,4,1,1}, {1,4,3,2,2,4}, {1,4,1,2,3,1}
             };
                 break;
              case 4:
                  map = new int[,] {
                 {1,2,4,2,2,1}, {2,1,1,4,1,1}, {2,1,4,1,3,1},
                 {2,1,1,2,1,4}, {4,1,3,1,1,4}, {1,2,2,2,4,1}
             };
                 break;
              case 5:
                  map = new int[,] {
                 {4,1,2,2,1,3}, {1,2,4,1,2,1}, {2,1,3,4,1,2},
                 {2,1,1,4,4,2}, {3,2,4,1,2,1}, {1,4,2,2,1,3}
             };
                 break;
              case 6:
                  map = new int[,] {
                 {1,2,3,1,4,1}, {4,2,1,4,2,1}, {4,2,3,1,2,4},
                 {2,1,3,1,4,2}, {2,1,4,1,3,2}, {1,2,2,2,4,1}
             };
                 break;
            }
        }
